...
if($result->num_rows > 0) {
	...
	$countOrderItemSql = "SELECT count(*) FROM orders_detalle_pdv WHERE order_id = $id_order";
 	$itemCountResult = $connect->query($countOrderItemSql);
 	$itemCountRow = $itemCountResult->fetch_row();

 	//Consulta para sumar el total de productos adquiridos por especie en total

 	/*$sumProduct = "SELECT sum(cantidad) FROM orders_detalle_pdv WHERE order_id = $id_order";
