...
if ($_POST) {
	...
	$id_user	 = $_SESSION["iduser"];
		$sql = "INSERT INTO orders_pdv (fecha_add, hora_add, client_id, total_neto, descuento, total, metodo, totalProducto, tipo_orden, monto, saldo, estado, fecha_liqui, user_add_id) 
...
/*		if($connect->query($sql) === TRUE) {
	...
	for($x = 0; $x < count($_POST['codProd']); $x++) {			
		$updateProductQuantitySql = "SELECT product.cantidad FROM product_pdv as product WHERE product.id_product = ".$_POST['codProd'][$x]."";
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
				...
				$updateProductTable = "UPDATE product_pdv SET cantidad = '".$updateQuantity[$x]."' WHERE id_product = ".$_POST['codProd'][$x]."";
				$connect->query($updateProductTable);

				// add into order_item
				$orderItemSql = "INSERT INTO orders_detalle_pdv (order_id, product_id, cantidad, precio, total) 
