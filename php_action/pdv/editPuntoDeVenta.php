...
if($_POST) {
	...
	$sql = "UPDATE orders_pdv SET client_id   	= '$a', 
							  total_neto  	= '$b',
							  descuento   	= '$c',
							  total       	= '$d',
							  metodo      	= '$e',
	...
	for($x = 0; $x < count($_POST['codProd']); $x++) {
		//  product table
		$updateProductCantidadSql = "SELECT product.cantidad FROM product_pdv as product WHERE product.id_product = ".$_POST['codProd'][$x]."";
		...
		while ($updateProductCantidadResult = $updateProductCantidadData->fetch_row()) {
			...
			$orderDetalleTableSql = "SELECT orders_detalle.cantidad FROM orders_detalle_pdv as orders_detalle WHERE orders_detalle.order_id = {$orderId}";
			...
			$updateCantidadSql = "UPDATE product_pdv SET cantidad = $editCantidad WHERE id_product = ".$_POST['codProd'][$x]."";
	...
	for($x = 0; $x < count($_POST['codProd']); $x++) {			
		$removeOrderSql = "DELETE FROM orders_detalle_pdv WHERE order_id = {$orderId}";
	...
	if($updateDetalleOrden) {
		...
		for($x = 0; $x < count($_POST['codProd']); $x++) {			
			...
			while ($updateProductCantidadResult = $updateProductCantidadData->fetch_row()) {
					...
					$updateProductTable = "UPDATE product_pdv SET cantidad = '".$updateCantidad[$x]."' WHERE id_product = ".$_POST['codProd'][$x]."";
					$connect->query($updateProductTable);

					// add into order_item
					$orderDetalleSql = "INSERT INTO orders_detalle_pdv (order_id, product_id, cantidad, precio, total) 
