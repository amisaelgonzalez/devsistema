...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			if(colina == "") {
				$("#colina").after('<p class="text-danger"> Este campo es obligatorio </p>');
				$('#colina').closest('.form-group').addClass('has-error');
			} else {
				$('#colina').closest('.form-group').addClass('has-success');
			} // /else

			if(calle == "") {
				$("#calle").after('<p class="text-danger"> Este campo es obligatorio </p>');
				$('#calle').closest('.form-group').addClass('has-error');
			} else {
				$('#calle').closest('.form-group').addClass('has-success');
			} // /else
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && colina && calle && ciudad && paid && discount && paymentType && paymentStatus) {
					...
					$.ajax({
						...
						success:function(response) {
							...
							if(response.success == true) {
				            	...
				            	'<a href="orders_users.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Agregar nueva orden </a>'+
				   		       '</div>');
...
function printOrder(orderId = null) {
	if(orderId) {		
		...
		$.ajax({
			url: 'php_action/printOrderUsers.php',
			type: 'post',
...
function removeOrder(orderId = null) {
	if(orderId) {
		$("#removeOrderBtn").unbind('click').bind('click', function() {
			...
			$.ajax({
				url: 'php_action/removeOrderUsers.php',
				type: 'post',
				data: {orderId : orderId},
				dataType: 'json',
				success:function(response) {
			...
			success:function(response) {				
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount && paymentType && paymentStatus) {
						...
						$.ajax({
							url: 'php_action/editPaymentUsers.php',
							type: 'post',
							data: {
								orderId: orderId,
								payAmount: payAmount,
